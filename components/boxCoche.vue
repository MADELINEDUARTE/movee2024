    ...
    <div class="car-item w-100 m-0">
        ...
        <div class="car-img">
            ...
            <div class="car-footer row justify-content-center" v-if="coche.config">
                ...
                <button :disabled="!coche.disponible?true:false" type="button" :style="{ width: plan_id == 1 ? '70%':'' }"   @click="selectCoche(1)" :class="plan_id == 1 ? 'col-auto' : 'col' " class="theme-btn1 py-2"><strong>{{ plan_id == 0 || plan_id == 1 ? 'Basic':'' }} {{ coche.config.basico }}{{$t('moneda')}}</strong></button>
                <button :disabled="!coche.disponible?true:false" type="button" :style="{ width: plan_id == 2 ? '70%':'' }"   @click="selectCoche(2)" :class="plan_id == 2 ? 'col-auto' : 'col' " class="theme-btn2 py-2"><strong>{{ plan_id == 0 || plan_id == 2 ? 'Medium':'' }} {{ coche.config.medio }}{{$t('moneda')}}</strong></button>
                <button :disabled="!coche.disponible?true:false" type="button" :style="{ width: plan_id == 3 ? '70%':'' }"   @click="selectCoche(3)" :class="plan_id == 3 ? 'col-auto' : 'col' " class="theme-btn3 py-2"><strong>{{ plan_id == 0 || plan_id == 3 ? 'Premium':'' }} {{ coche.config.premium }}{{$t('moneda')}}</strong></button>
